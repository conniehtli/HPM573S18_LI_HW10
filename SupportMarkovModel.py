...
def print_comparative_outcomes(simOutputs_no, simOutputs_ac):
    ...
    if Settings.PSA_ON:
        increase_survival_time = Stat.DifferenceStatPaired(
            ...
            x=simOutputs_ac.get_survival_times(),
            y_ref=simOutputs_no.get_survival_times())
    else:
    ...
    if Settings.PSA_ON:
        increase_discounted_cost = Stat.DifferenceStatPaired(
            ...
            x=simOutputs_ac.get_costs(),
            y_ref=simOutputs_no.get_costs())
    else:
    ...
    if Settings.PSA_ON:
        increase_discounted_utility = Stat.DifferenceStatPaired(
            ...
            x=simOutputs_ac.get_utilities(),
            y_ref=simOutputs_no.get_utilities())
    else:
    ...
    if Settings.PSA_ON:
        decrease_number_of_strokes = Stat.DifferenceStatPaired(
            name='Decrease in number of strokes',
            x=simOutputs_ac.get_if_developed_stroke(),
            y_ref=simOutputs_no.get_if_developed_stroke())
    else:
        decrease_number_of_strokes = Stat.DifferenceStatIndp(
            name='Decrease in number of strokes',
            x=simOutputs_ac.get_if_developed_stroke(),
            y_ref=simOutputs_no.get_if_developed_stroke())
    ...
    # estimate and CI
    estimate_CI = F.format_estimate_interval(
        estimate=decrease_number_of_strokes.get_mean(),
        interval=decrease_number_of_strokes.get_t_CI(alpha=Settings.ALPHA),
        deci=2)
    print("Average decrease in expected number of strokes "
        "and {:.{prec}%} confidence interval:".format(1 - Settings.ALPHA, prec=0),
            estimate_CI)


def report_CEA_CBA(simOutputs_no, simOutputs_ac):
    """ performs cost-effectiveness analysis
    :param simOutputs_no: output of a cohort simulated under no therapy
    :param simOutputs_ac: output of a cohort simulated under anticoaguation therapy
    """

    # define two strategies
    no_therapy_strategy = Econ.Strategy(
        name='No Therapy',
        cost_obs=simOutputs_no.get_costs(),
        effect_obs=simOutputs_no.get_utilities()
    )
    ac_therapy_strategy = Econ.Strategy(
        name='Anticoagulation Therapy',
        cost_obs=simOutputs_ac.get_costs(),
        effect_obs=simOutputs_ac.get_utilities()
    )

    # CEA
    if Settings.PSA_ON:
        CEA = Econ.CEA(
            strategies=[no_therapy_strategy, ac_therapy_strategy],
            if_paired=True
        )
    else:
        CEA = Econ.CEA(
